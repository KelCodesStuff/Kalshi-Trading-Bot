...
- [ ] **Zero Orphaned Orders:** Manually trigger the Kill Switch (press `Ctrl+C`) during an active trading session. Verify via the Kalshi UI that absolutely zero open orders were left behind.
- [ ] **State Recovery Verification:** Simulate a sudden server failure by using `kill -9` on the bot's process. Restart the bot and verify its initialization sequence correctly finds and cancels the old resting orders before quoting again.
